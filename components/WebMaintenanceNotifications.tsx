...
/**
 * WebMaintenanceNotifications - A web-compatible version of the maintenance notifications
 * that doesn't rely on expo-notifications, but still provides visual alerts
 * in the dashboard for upcoming and overdue maintenance.
 */
...
interface WebMaintenanceNotificationsProps {
  limit?: number;
  showHeader?: boolean;
  onViewAllPress?: () => void;
}

export default function WebMaintenanceNotifications({
  limit = 3,
  showHeader = true,
  onViewAllPress
}: WebMaintenanceNotificationsProps) {
      ...
      if (data) {
        data.forEach((device: any) => {
          ...
          if (isOverdue || isUpcomingSoon) {
            ...
            if (device.device_categories && typeof device.device_categories === 'object') {
              categoryName = device.device_categories.name || "Unknown";
            }
